...
<body>
    <div class="navbar">
        ...
        <div class="nav-buttons">
            ...
            <a href="/admin/{{ admin_secret }}/logout" class="nav-btn">خروج</a>
        </div>
    </div>
    ...
    <div class="container">
        <div class="stats">
            <div class="stat">
                <span class="stat-num">{{ stats.platforms }}</span>
                منصات
            </div>
            <div class="stat">
            ...
            <div class="stat">
                <span class="stat-num">{{ stats.codes }}</span>
                أكواد
            </div>
            <div class="stat">
...
def admin_settings():
    if request.method == 'POST':
        for key in ['main_color', 'secondary_color', 'background_color', 'text_color', 'ticker_text']:
            value = request.form.get(key)
            if value:
                update_text(key, value) if key == 'ticker_text' else set_setting(key, value)
    ...
    <head>
        ...
        <style>
            * { margin:0; padding:0; box-sizing:border-box; }
            ...
            .navbar { background:linear-gradient(135deg, #1f6feb, #388bfd); padding:20px; }
            .container { max-width:800px; margin:0 auto; padding:30px 20px; }
            .form-group { margin-bottom:20px; }
            label { display:block; margin-bottom:8px; font-weight:bold; color:#1f6feb; }
            input { width:100%; padding:12px; background:#161b22; border:1px solid #30363d; color:#c9d1d9; border-radius:8px; }
            button { padding:12px 25px; background:#1f6feb; color:white; border:none; border-radius:8px; cursor:pointer; }
            button:hover { background:#388bfd; }
            .nav-btn { padding:10px 20px; background:rgba(255,255,255,0.1); color:white; text-decoration:none; border-radius:8px; }
        </style>
    </head>
    <body>
        <div class="navbar">
            <h1 style="color:white;">⚙️ الإعدادات</h1>
            <a href="/admin/admin_secret_77" style="color:white; text-decoration:none; margin-top:10px; display:inline-block;">← العودة</a>
        </div>
        <div class="container">
            <form method="POST">
                <div class="form-group">
                    <label>🎨 اللون الأساسي</label>
                    <input type="color" name="main_color" value="{{ main_color }}">
                </div>
                <div class="form-group">
...
@app.route('/admin/' + ADMIN_SECRET_PATH + '/texts', methods=['GET', 'POST'])
@admin_required
def admin_texts():
    if request.method == 'POST':
    ...
    <head>
        ...
        <style>
            * { margin:0; padding:0; box-sizing:border-box; }
            ...
            .form-group { margin-bottom:20px; }
            label { display:block; margin-bottom:8px; font-weight:bold; color:#1f6feb; }
            textarea { width:100%; padding:12px; background:#161b22; border:1px solid #30363d; color:#c9d1d9; border-radius:8px; min-height:100px; }
            button { padding:12px 25px; background:#1f6feb; color:white; border:none; border-radius:8px; cursor:pointer; }
        </style>
    </head>
    <body>
        <div class="navbar">
            <h1 style="color:white;">📝 النصوص</h1>
            <a href="/admin/admin_secret_77" style="color:white; text-decoration:none; margin-top:10px; display:inline-block;">← العودة</a>
        </div>
        <div class="container">
            <form method="POST">
                <div class="form-group">
                    ...
                    <textarea name="site_title">""" + get_text('site_title') + """</textarea>
                </div>
                <div class="form-group">
                    <label>الوصف</label>
                    <textarea name="site_subtitle">""" + get_text('site_subtitle') + """</textarea>
                </div>
                <div class="form-group">
                    <label>نص الشريط المتحرك</label>
                    <textarea name="ticker_text">""" + get_text('ticker_text') + """</textarea>
                </div>
                <div class="form-group">
                    <label>نص التذييل</label>
                    <textarea name="footer_text">""" + get_text('footer_text') + """</textarea>
                </div>
                <button type="submit">💾 حفظ</button>
            </form>
        </div>
...
@app.route('/admin/' + ADMIN_SECRET_PATH + '/links', methods=['GET', 'POST'])
@admin_required
def admin_links():
    if request.method == 'POST':
        ...
        if key and value:
            update_link(key, value, icon)
    
    links = get_all_links()
    links_html = ''.join([f"<tr><td>{l[0]}</td><td>{l[1]}</td><td>{l[2]}</td></tr>" for l in links])
    ...
    <body>
        <div class="navbar">
            <h1 style="color:white;">🔗 الروابط</h1>
            <a href="/admin/admin_secret_77" style="color:white; text-decoration:none; margin-top:10px; display:inline-block;">← العودة</a>
        </div>
        <div class="container">
            <form method="POST">
            ...
            <h3 style="margin-top:30px;">الروابط الحالية</h3>
            <table>
                <tr><th>المفتاح</th><th>الرابط</th><th>الأيقونة</th></tr>
                {links_html}
            </table>
        </div>
    </body>
...
@app.route('/admin/' + ADMIN_SECRET_PATH + '/combos', methods=['GET'])
@admin_required
def admin_combos():
    combos = get_all_combos()
    combos_html = ''.join([f"<tr><td>{c[0]}</td><td>{c[2]}</td><td>{c[3]}</td></tr>" for c in combos])
    ...
    <body>
        <div class="navbar">
            <h1 style="color:white;">📱 الأرقام والمنصات</h1>
            <a href="/admin/admin_secret_77" style="color:white; text-decoration:none; margin-top:10px; display:inline-block;">← العودة</a>
        </div>
        <div class="container">
            <h3>الأرقام الموجودة</h3>
            <table>
                <tr><th>المنصة</th><th>الدولة</th><th>العلم</th></tr>
                {combos_html}
            </table>
        </div>
    </body>
...
@app.route('/admin/' + ADMIN_SECRET_PATH + '/codes', methods=['GET'])
@admin_required
def admin_codes():
    codes = get_recent_otps(50)
    codes_html = ''.join([f"<tr><td>{c[1]}</td><td>{c[2]}</td><td>{c[3]}</td><td>{c[4]}</td></tr>" for c in codes])
    ...
    <body>
        <div class="navbar">
            <h1 style="color:white;">🔐 آخر الأكواد</h1>
            <a href="/admin/admin_secret_77" style="color:white; text-decoration:none; margin-top:10px; display:inline-block;">← العودة</a>
        </div>
        <div class="container">
            <table>
                <tr><th>الرقم</th><th>الكود</th><th>الوقت</th><th>المنصة</th></tr>
                {codes_html}
            </table>
        </div>
    </body>
...
@app.route('/admin/' + ADMIN_SECRET_PATH + '/logs', methods=['GET'])
@admin_required
def admin_logs():
    return """
    <!DOCTYPE html>
    ...
    <body>
        <div class="navbar">
            <h1 style="color:white;">📊 السجلات</h1>
            <a href="/admin/admin_secret_77" style="color:white; text-decoration:none; margin-top:10px; display:inline-block;">← العودة</a>
        </div>
        <div class="container">
            <p>قيد التطوير...</p>
